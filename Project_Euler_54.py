...
for i in range(len(chunks)):
    ...
    for i in range(0, len(hand),3):
        if len(player1Num) < 5:
             player1Num.append(hand[i])
        else:
            player2Num.append(hand[i])

    for i in range(1,len(hand), 3):
        if len(player1Suit) < 5:
            player1Suit.append(hand[i])
        else:
            player2Suit.append(hand[i])

    def changeHand(hand):
        arr = ["10" if i == "T" else i for i in hand]
        arr = ["11" if i == "J" else i for i in arr]
        arr = ["12" if i == "Q" else i for i in arr]
        arr = ["13" if i == "K" else i for i in arr]
        arr = ["14" if i == "A" else i for i in arr]
        return arr
    ...
    def twoPair(arr):
        ...
        for i in range(len(arr)):
            if arr.count(arr[i]) == 2 and arr[i] not in hold:
                hold.append(arr[i])
                count += 1
        if count == 2:
            hold.sort()
    ...
    def flush(arrSuit,arrNums):
        if arrSuit.count(arrSuit[0]) == 5:
            return arrSuit[0], "5"
        return straight(arrNums)

    def fullHouse(arrSuit, arrNums):
        arrNums.sort()
        if arrNums.count(arrNums[0]) == 2 and arrNums.count(arrNums[4]) == 3:
            return arrNums[4], "6"
        elif arrNums.count(arrNums[0]) == 3 and arrNums.count(arrNums[4]) == 2:
            return arrNums[0], "6"
        return flush(arrSuit, arrNums)

    def fourOfAKind(arrSuit, arrNums):
        if arrNums.count(arrNums[0]) == 4:
            return arrNums[0], "7"
        elif arrNums.count(arrNums[1]) == 4:
            return arrNums[1], "7"
        return fullHouse(arrSuit, arrNums)

    def straightFlush(arrSuit, arrNums):
        arrNums.sort()
        start = arrNums[0]
        for a in arrNums:
            if a == start:
                ...
                return fourOfAKind(arrSuit, arrNums)
        # Royal Flush
        if arrSuit.count(arrSuit[0]) == 5 and arrNums[4] == 14:
            return arrNums[4], "9"
        elif arrSuit.count(arrSuit[0]) == 5:
            return arrNums[4], "8"
        return fourOfAKind(arrSuit,arrNums)
    ...
    player1Ints = [int(i) for i in changeHand(player1Num)]
    player2Ints = [int(i) for i in changeHand(player2Num)]

    x = straightFlush(player1Suit, player1Ints)
    y = straightFlush(player2Suit, player2Ints)

    if int(x[1]) > int(y[1]):
        player1Wins += 1
    elif int(x[1]) == 5 and int(y[1]) == 5:
        i = highCard(player1Suit, player1Ints)
        j = highCard(player2Suit, player2Ints)
        if int(i[0]) > int(j[0]):
            player1Wins += 1
        ...
        if x[0] > y[0]:
            ...
            i = highCard(player1Ints)
            j = highCard(player2Ints)
            if i[0] > j[0]:
                player1Wins += 1
